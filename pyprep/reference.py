...
class Reference:
    ...
    def perform_reference(self):
        ...
        self.reference_signal = np.nanmean(self.raw.get_data(picks=self.reference_channels), axis=0) * 1e6
        ...
        self.raw.interpolate_bads()
        reference_correct = np.nanmean(self.raw.get_data(picks=self.reference_channels), axis=0) * 1e6
        self.EEG = self.raw.get_data() * 1e6
    ...
    def robust_reference(self):
        ...
        self.unusable_channels = union(noisy_detector.bad_by_nan, noisy_detector.bad_by_flat)
        # unusable_channels = union(unusable_channels, noisy_detector.bad_by_SNR)
        self.reference_channels = set_diff(self.reference_channels, self.unusable_channels)

        # Get initial estimate of the reference by the specified method
        signal = self.raw.get_data() * 1e6
        self.reference_signal = np.nanmedian(self.raw.get_data(picks=self.reference_channels), axis=0) * 1e6
        reference_index = [self.ch_names_eeg.index(ch) for ch in self.reference_channels]
        ...
        while True:
            ...
            noisy_detector.find_all_bads(ransac=self.ransac)
            self.noisy_channels['bad_by_nan'] = union(self.noisy_channels['bad_by_nan'], noisy_detector.bad_by_nan)
            self.noisy_channels['bad_by_flat'] = union(self.noisy_channels['bad_by_flat'], noisy_detector.bad_by_flat)
            self.noisy_channels['bad_by_deviation'] = union(self.noisy_channels['bad_by_deviation'], noisy_detector.bad_by_deviation)
            self.noisy_channels['bad_by_hf_noise'] = union(self.noisy_channels['bad_by_hf_noise'], noisy_detector.bad_by_hf_noise)
            self.noisy_channels['bad_by_correlation'] = union(self.noisy_channels['bad_by_correlation'], noisy_detector.bad_by_correlation)
            self.noisy_channels['bad_by_ransac'] = union(self.noisy_channels['bad_by_ransac'], noisy_detector.bad_by_ransac)
            self.noisy_channels['bad_all'] = union(self.noisy_channels['bad_all'], noisy_detector.get_bads())
            logger.info('Bad channels: {}'.format(self.noisy_channels))

            if iterations > 1 and (not self.noisy_channels['bad_all'] or
                                   set(self.noisy_channels['bad_all']) == set(noisy_channels_old)) or \
            ...
            if self.noisy_channels['bad_all']:
                ...
                signal_tmp = signal
            self.reference_signal = np.nanmean(raw_tmp.get_data(picks=self.reference_channels), axis=0) * 1e6
