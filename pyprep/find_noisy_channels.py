...
class NoisyChannels():
    ...
    def get_bads(self, verbose=False):
        ...
        bads = (self.bad_by_nan + self.bad_by_flat + self.bad_by_deviation + self.bad_by_hf_noise + self.bad_by_SNR +
    ...
    def find_bad_by_deviation(self, deviation_threshold=5.0):
        ...
        for i in range(0, self.new_dimensions[0]):
            deviation_channel_mask[i] = abs(robust_channel_deviation[i]) > deviation_threshold or np.isnan(
    ...
    def find_bad_by_hfnoise(self, HF_zscore_threshold=5.0):
        ...
        if self.sample_rate > 100:
            ...
            for i in range(0, self.new_dimensions[0]):
                HFnoise_channel_mask[i] = zscore_HFNoise[i] > HF_zscore_threshold or np.isnan(
                    zscore_HFNoise[i])
    ...
    def find_bad_by_correlation(self,correlation_secs=1.0, correlation_threshold=0.4, frac_bad=0.01):
        ...
        correlation_offsets = np.arange(1, self.new_dimensions[1] - correlation_frames,
                                        correlation_frames)
        ...
        EEG_new_win = np.reshape(
            ...
            order="F")
        data_win = np.reshape(
            np.transpose(self.EEGData_beforeFilt[0: len_correlation_window * w_correlation, :]),
            (self.new_dimensions[0], len_correlation_window, w_correlation),
            order="F")
        ...
        for i in range(0, w_correlation):
            for j in range(0, self.new_dimensions[0]):
                ...
                if drop[i, j] == 1:
                    ...
                    noiselevels[i, j] = 0
        maximum_correlations[self.channels_interpolate, :] = np.transpose(channel_correlation)
        ...
        bad_correlation_channels_idx = np.argwhere(fraction_BadCorrelationWindows > frac_bad)
    ...
    def find_bad_by_SNR(self):
        ...
        self.bad_by_SNR=self.bad_by_hf_noise+self.bad_by_correlation
        return None
    ...
    def find_bad_by_ransac(self, n_samples=50, fraction_good=0.25,
        ...
        ransac_eeg = self.run_ransac(chn_pos=chn_pos,
    ...
    def run_ransac(self, chn_pos, chn_pos_good, good_chn_labs, n_pred_chns, data, n_samples):
        ...
        for sample in range(n_samples):
            eeg_predictions[..., sample] = self.get_ransac_pred(chn_pos,
                                                                 chn_pos_good,
                                                                 good_chn_labs,
                                                                 n_pred_chns,
                                                                 data)
