...
from pyprep.utils import filter_design, mat_quantile, mat_iqr
...
class NoisyChannels:
    ...
    def find_bad_by_deviation(self, deviation_threshold=5.0):
        ...
        for i in range(0, self.new_dimensions[0]):
            channel_deviation[i] = 0.7413 * mat_iqr(self.EEGData[i, :])
        channel_deviationSD = 0.7413 * mat_iqr(channel_deviation)
    ...
    def find_bad_by_correlation(
        ...
        for k in range(0, w_correlation):
            ...
            channel_correlation[k, :] = mat_quantile(abs_corr, 0.98, axis=0)
            noiselevels[k, :] = np.divide(
                robust.mad(np.subtract(data_portion, eeg_portion), c=1),
                robust.mad(eeg_portion, c=1),
            )
            channel_deviations[k, :] = 0.7413 * mat_iqr(data_portion, axis=0)
